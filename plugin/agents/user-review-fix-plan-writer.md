...
2. **For each numbered observation, identify its reference type and resolve it:**
   ...
   - **File and line** (e.g., "`<file>:<line>` ...") → read the file directly, resolving its path against `<repo_root>` — the user may cite a script, a config or any other file, so the citation is not scoped to the application's own tree. Store what that line holds, never the coordinate alone: the finding's site anchor names the symbol, heading or quoted substring you found there.
...
5. **Quality checks before saving:**
   ...
   - Every finding's detail file carries a site anchor you grep-verified in the current tree.
