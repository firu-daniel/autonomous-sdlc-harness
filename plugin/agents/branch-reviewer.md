...
3. **Reconcile each candidate against Pass 1:**
   - **Already covered:** Pass 1 already flagged the same root issue (the same file and line, or the same systemic problem at a different location). Skip — do not duplicate.
   - **Stale:** the issue described no longer applies in the current diff (a later unit refactored the file, the line moved, the component was deleted). Skip.
