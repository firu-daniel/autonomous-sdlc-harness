...
3. **Check whether a plan already exists** by testing for the **story index** at `<state_dir>/story_plans/<branch>_story_plan.md`.
   - If yes: present the existing plan (the story index's readiness list + the per-task files under `<state_dir>/task_plans/<branch>/`) to the user and ask whether to **extend it** (add missing tasks), **rewrite from scratch** (the existing one is wrong/stale), or **stop** (the existing plan is good — just run `/branch-implement-plan` next). Do not silently overwrite.
   - If no: continue.
