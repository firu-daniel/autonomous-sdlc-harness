...
**Usage:** type `/branch-prompt` and then **keep typing your task description on the same line** — do NOT hard-select the entry from the slash menu; just continue the line. Everything after the command name is `$ARGUMENTS`. Example:
`/branch-prompt Add a search field to the settings screen, with debounced input and an empty state.`
...
6. Report: the exact file written, the `<branch>` the watcher will create the worktree for, and that `/branch-status <branch>` gives a progress digest. State explicitly that this command does **not** launch the run or send notifications — the watcher does both.
