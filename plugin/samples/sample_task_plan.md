...
> **This file is the split-plan format reference for every command and agent that dereferences it by path** — the `/branch-start-plan` command, the `task-plan-writer` that emits a plan, the `task-plan-reviewer` that grades one, and the `layer-reviewer` that checks a sample fixture against the shape its writer agent must emit. It explains the thin-index-plus-per-item split and links the concrete templates. It deliberately carries **no** plan body of its own — the body lives in the per-task templates linked at the end.
>
