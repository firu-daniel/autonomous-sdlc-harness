...
| Planning **core** (mode-free: Setup, safety contract, the writer/reviewer loop, both plan gates, the interactive-test-plan loop, convergence) | `${CLAUDE_PLUGIN_ROOT}/instructions/task_plan_writing_instructions_core.md` |
...
A run that reaches a decision it must **not** take alone writes a self-contained question as `question_<n>.md` under `<state_dir>/clarifications/<branch>/` in its own working copy, and **ends its session**. That is the whole of the run's side. It is not a stall and it costs nothing while it waits: the session is over.
...
The paired `answer_<n>.md` is written beside the question — pairing is by index — and the detection, the re-launch of the same engine in the same working copy, and the archiving of the answered pair are the watcher's (the harness repository's `docs/watcher.md` §4). The resumed session consumes the pair; nothing removes a question ahead of the run that has to read it.
