...
| `<reentry_command>` | `/branch-implement-user-review-semi-autonomous` |
| `<next_step_note>` | **Optional next round.** If the user has further observations after this fix cycle, they write `<state_dir>/user_reviews/<branch>_review_2.md` (round 2) following `${CLAUDE_PLUGIN_ROOT}/samples/sample_user_review.md`, then run `/branch-start-user-review-fix-plan` (supervised) to produce `<branch>_fix_plan_2.md`, then re-run `/branch-implement-user-review-semi-autonomous`. This orchestrator does NOT auto-trigger that next round — the user invokes it out-of-band when (and if) they want it. |
