...
4. [x] **Finding 6** — Emit no chunk for an empty `##` section that has `###` children. _(layer: cli, general)_
5. [x] **Finding 3** — Opt-in `search_docs` query logging behind one environment variable. _(layer: cli, general)_
6. [x] **Finding 2** — Rename the adopter-facing surface to "RAG", leaving every identifier alone. _(layer: cli, plugin, general)_
