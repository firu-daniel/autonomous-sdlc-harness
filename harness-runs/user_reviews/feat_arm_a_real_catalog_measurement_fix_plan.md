...
2. [x] **Finding 2** — Rewrite roadmap item 18 in `docs/development.md` §6 as the record that the withdrawal was not executed, and name item 18 as neither shipped nor owed in the paragraph below the table. _(layer: general)_
3. [x] **Finding 3** — Rewrite `docs/retrieval.md`'s opening paragraph and add `## When to turn it on`: the unmeasured situations, the `lexical`-above-`fused-rerank` disclosure, and a warning that it is not for product search. _(layer: general)_
4. [x] **Finding 4** — Correct the `## Threshold calibration` and `## The shipped default against fusion alone` passages that say the withdrawal removes the constant or every mode. _(layer: general)_
5. [x] **Finding 5** — Sweep the remaining twelve withdrawn/removal statements (README, llms.txt, schema description, config.md, evals/README, ROADMAP ×2, cli.md §11, cli/README, queryLog.ts header) to "measured, verdict withdrawn, kept opt-in by maintainer decision". _(layer: cli, general)_
