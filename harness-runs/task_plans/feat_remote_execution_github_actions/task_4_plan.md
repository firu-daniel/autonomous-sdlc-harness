...
- The bundle names and `HR_REMOTE_STATE_SCHEMA` are assigned by a function, `hr_remote_names_var`, that every bundle function calls first. They are not top-level assignments. The library header's `FILE DISCIPLINE` forbids top-level side effects, and its `NAMING` paragraph requires every `HR_` variable to be assigned before the function that owns it reads it, the same way `hr_protected_default_var` / `hr_command_keys_var` work. The source name `.flow_walker_state` is built as `".$HR_REMOTE_WALKER_FILE"`, so the dotless name is spelled once.
- The header's environment-read list also names `HARNESS_INPUT_CHAIN`, alongside `GITHUB_RUN_ID` / `GITHUB_SERVER_URL` / `GITHUB_REPOSITORY`. `hr_remote_status_write` reads it for `chain`, and `JURISDICTION` requires every environment read to be listed.
- `hr_remote_status_write` returns 1 and writes nothing in three cases: `<decision>` is outside `continue|wait-poller|stop`; the record's `status` is outside the registry vocabulary (this includes no record at all); or an argument is missing. A registry `pause_reason` outside `usage|budget|user|overload` (for example `killed`) is written as an empty string. A `HARNESS_INPUT_CHAIN` that is not all digits is also written as an empty string. `run_url` is built from `GITHUB_SERVER_URL/GITHUB_REPOSITORY/actions/runs/GITHUB_RUN_ID` only when all three are set; otherwise it is empty. Carriage returns and newlines in `detail` are turned into spaces.
- `hr_remote_bundle_write`: when `autonomous_logs/remote_status.json` is absent, `status.json` is written from the registry record with decision `stop`. A job that never wrote its status never chose to continue, and this is what the `<registry_file>` argument is used for. `<out_dir>` must be absent or empty; otherwise the call returns 1, because `cp -R` into a directory that already exists would nest the copy. It returns 2 when `<root>`'s configuration cannot be resolved.
- `hr_remote_bundle_restore` also returns 2, touching nothing, in two more cases: the bundle's `status.json` names a different `branch`, or `<root>`'s configuration cannot be resolved. It returns 1 for a missing argument or an unknown `<mode>`. A bundle that carries no clarification directory leaves the target directory alone, because in a mirror that directory can hold an answer not yet relayed. When two restores happen in the same epoch second, the move-aside directory is `<epoch>-<n>`.
- Evidence downgrade: the plan's "no partial file under concurrent read" is shown by a second write installing a new inode (a rename, not an in-place rewrite) and by no `.tmp.` file left beside `status.json`. No actual concurrent reader was run.
