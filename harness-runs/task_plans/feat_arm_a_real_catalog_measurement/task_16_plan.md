...
- [x] **Find the answer** exactly as Task 15 does: `### Operator answer` above, else an `answer_<n>.md` beside a `question_<n>.md` naming **Task 16** under `harness-runs/clarifications/feat_arm_a_real_catalog_measurement/` or its `answered/`. None → the next two bullets; one → the last two.
...
- [x] **On the answer: check it against the tree.** Every transcript the answer reports exists under `harness-runs/scratch/arm-a/gate10-catalog/` with one record per query id of the set (a stopped variant has fewer repetitions — accepted, recorded as partial); the catalog's commit is still the one recorded before the park and `docs/` still matches `57a6c25` (launcher (e) again) — if either moved, stop and return a blocker naming it, because the B–E run must be at the arm A snapshot.
- [x] **On the answer: record the run.** Rename the heading to `## Arm A — the real-catalog hand run` and replace its body with: what arm A measures (kept from the old text, reworded for two variants); the catalog by commit and size; the date; the host stamp in this tree's shape (`darwin <release>` plus the machine), the agent CLI version and the model; per variant and per pass, done or stopped; the 5-hour and 7-day window utilisation before and after **at the granularity the operator read it** — per repetition pair, per `### Operator answer` item 1 — and the operator time; the cost checkpoint's outcome, **recorded as a cost result** (passed here, per item 2); the flags verified; the catalog-root files found in (d) with what each session loaded, per item 4 (which replaces the park's hook description); the three prose-beside-`none` records, per item 5; and that the figures follow in the next section. Re-point the two citations in `docs/retrieval-eval.md`.
  - **Tree check, as run on resume** (`bash scripts/scratch-run.sh harness-runs/scratch/task16_resume.mjs`): all ten files present, each 69 records with 69 distinct ids in the set's own order, 690 in total; `variant` matches the file name; no ref carries a code fence; `toolCalls` names `Read`,`Grep` in every index pass and `Read`,`Grep`,`Glob` in every search pass; `scoreTranscript` reads each of the three prose-beside-`none` records as `abstained false`, 2 hits — agreeing with the operator; launcher (e) printed `catalog HEAD 57a6c25; diff --quiet 57a6c25 -- docs exit 0`. Catalog root (d), re-listed: `.claude/CLAUDE.md`, `.claude/settings.json`, `.claude/settings.autonomous.json`, `.mcp.json`; no root `CLAUDE.md`; `docs/expause-web/INDEX.md` present.
  - **Deviations from plan:**
    - The operator's "per-session cache writes" (3.20 M / 1.70 M, then ~1.1–1.4 M / ~0.35 M) are **per-pass totals**: summing `usage.cache_creation_input_tokens` per file (`harness-runs/scratch/task16_cachewrites.mjs`) gives index-rep1 3,198,348, search-rep1 1,703,099, index-rep2–5 1,145,134–1,357,514, search-rep2–5 302,858–372,653. Recorded as per-pass totals, still attributed to the operator.
    - The record keeps a short **Why an operator ran it** paragraph from the old body, because `docs/retrieval-eval.md:503` cites this section as "the record of that decision", and opens with a who-reads-this line.
    - The flag check is the pre-park dispatch's (`question_3.md`: all seven flags in `--help`, `run-arm-a.sh` unchanged); it was not re-run on resume, since this task's only agent-CLI call is that one `--help`.
    - Verification's `git grep -n 'Arm A — awaiting'` still matches point-in-time artifacts under `harness-runs/` (earlier branches' plans and reviews, this branch's task prompt, story plan and task plans); it returns nothing outside `harness-runs/`, and the wrapped half `^a hand run` returns nothing.
