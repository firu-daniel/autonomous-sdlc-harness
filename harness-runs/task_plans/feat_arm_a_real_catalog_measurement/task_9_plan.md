...
- The `jq` 1.5 floor rests on reading, not execution: the only `jq` on this host is 1.8.2, which is the one the stand-in run exercised. The stream parse uses only builtins present in 1.5 (`select`, `objects`, `reduce`, `.[-1]`, `//`, `[]?`). The `bash` 3.2 floor was executed: the scratch driver spawned `/bin/bash` 3.2.57.
- The exit-65 case ran against a scratch **copy** of `run-arm-a.sh` next to scratch task files, because the runner finds its task files next to its own path (`BASH_SOURCE`) and the contract has no option to override that.
