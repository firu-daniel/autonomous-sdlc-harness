...
- Rounding strips float noise with `Number((x * 100).toPrecision(12))` before `Math.round`, instead of the plan's example `Math.round(x * 100 + Number.EPSILON)`: `Number.EPSILON` (~2.2e-16) is below half the spacing of doubles near `32` (spacing 2^-47, ~7.1e-15), so adding it to a product in that range changes nothing. Both give `0.32` on the worked value `0.3195` (`0.3195 * 100` is exactly `31.95` in V8, measured by the launcher). The overlapping value ("smallest two-decimal number strictly greater") is taken without noise-stripping and then checked as the doubles `best < ABSTAIN_SCORE_THRESHOLD` compares, so `0.29` yields `0.30`, not `0.29`.
- `price` is computed against the value on every method (on `cannot-separate`, against the candidate value, so the refusing price is published by id), and `nearBelowL` whenever `L` exists; the interface fixed both fields without saying on which methods they are populated. A pool with no scored positive or no scored negative is refused by name, since neither half of the method is defined over it.
- The plan's `git grep` literal check returns nothing on an untracked `calibrate.mjs` regardless of content; it was run as `grep -nE` over the file instead (no hit; `STRICT_GRADE` and `NEGATIVE_KINDS` imports present).
