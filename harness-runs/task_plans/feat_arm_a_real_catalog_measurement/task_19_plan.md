...
- [ ] **`### How arm A navigated a 156-file catalog`** — per variant: tool calls per query by tool, **whether A-index used `Grep` and on how many queries** (A-index could grep; the task prompt asks whether it did), any out-of-fence tool Task 17 flagged; how often it gave up (`none` on a positive), guessed (refs naming no chunk, `unresolvedRefs`), or answered. Name the three records with a prose sentence beside `none` (Task 16's `### Operator answer` item 5) and state how `score-transcript.mjs` scored each one. Per the operator, it scores a `none` among other refs as an answered miss, so `search-rep5` `q-g10-neg-android-keystore`, a `far` negative, costs A-search one abstention on repetition 5. Verify that against the launcher's output rather than asserting it; and the per-half result read against A-index's index not linking `docs/vite/` — the variant's real behaviour on a mixed catalog, reported, not corrected.
- [ ] **`### What arm A cost to run`** — per variant: billed tokens per field and in total, per query (median, p95), the **five costliest queries** by billed tokens with their ids (the per-query `usage` blocks themselves are the committed transcripts — cite the directory, do not paste them), the 5-hour and 7-day window rise from Task 16's record **per repetition pair**, the granularity the operator read it at (Task 16's `### Operator answer` item 1). Never apportion a pair's rise to one variant as a measured figure. Rep 1's ~63% / ~37% split is quoted only as the operator's token-weighted estimate, beside the billed-token split this launcher computes from the transcripts. Report the warm-cache drop from rep 1's +15 to +6 per later pair. Add the operator time — beside what the run settled, so the next person deciding whether to re-run it on another catalog has the price. No money figure (the transcript keeps tokens, not a charge).
