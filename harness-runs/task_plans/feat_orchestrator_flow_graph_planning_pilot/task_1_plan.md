...
- The skip orders match the core's textual order for both loops, as argued above. **Task 12**'s schema validates this file, and **Task 13**'s checker runs its seven static checks over it, one of which (`cap-matches-core`) requires this file's `counters.iteration.cap` to equal the cap the core's `## Setup` step 4 and `## Safety contract` step 2 still state.
