...
- `PUSH_DESTINATION_FORMS` is a noun phrase beginning "either …" with no final stop, not a free-standing sentence: Task 3 interpolates it as `Type ${PUSH_DESTINATION_FORMS}.` and `--push-url takes ${PUSH_DESTINATION_FORMS}; the value given is neither`, and only that shape reads correctly in both places. It quotes `GUIDED_ENDPOINT_EXAMPLE` as what a topic becomes.
- `bash scripts/test.sh` exited 1, not 0. Gate 4 (`npm test`, which includes every `cli/test` suite and the new cases) passed. The three failing gates have inputs this diff does not touch: `1a plugin manifest` (strict-mode warnings on unquoted `${CLAUDE_PLUGIN_ROOT}` in `plugin/hooks/hooks.json`), `6a no machine paths` (the worktree's untracked `.git` pointer file and `harness-runs/` artifacts quoting it), and `11 docs-retrieval relevance floor` (the retrieval runtime is not installed on this host). The "both exit 0" bullet therefore rests on gate 4's pass plus that attribution, not on a clean exit of the whole script.
