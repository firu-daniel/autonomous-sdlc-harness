...
1. [ ] **Finding 1** — Drop the false "quoted by the engine's Override 2(a)" sentence from the watcher's resume-clause comment, in the template and its mirror _(layer: cli, general)_
2. [ ] **Finding 3** — Restate the park signal as "a question file left at the top level" in `docs/watcher.md` §4 and the clarifications README pair _(layer: cli, general)_
3. [ ] **Finding 4** — Scope the mode contract's Class (i-b) and the owning fork's "nothing out of scope is stranded" claims to shipped files, naming non-shipped citers of the channel anchor only as examples _(layer: plugin)_
