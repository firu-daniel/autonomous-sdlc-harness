...
- [ ] In `docs/retrieval.md` → `## Still open` → the lexical-arm entry, replace *"at 40, 166 and 1024 rows, which covers both committed corpora's sizes and one far above them"* with: *"at 40, 166 and 1024 synthetic rows, which bracket both committed corpora's recorded sizes — 41 and 177 chunks — and reach far above them"*.
