# Branch statistics: fix_observations_repo_relative_paths

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `45` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
- `issue_cost_total`: `0` — subtraction (sum of each user-review observation's severity weight, in story-point units).
- `story_plan_tasks`: `5` — retained raw count of `## Phase 2 Readiness — Ordered Fix List` task entries.
- `user_review_issues`: `0` — retained raw count of observations across all `fix_observations_repo_relative_paths_review*.md` user-review files.
...
Worked example: `round(clamp((45 - 0) / 45, 0, 1) * 100, 1)` = `100%` (and the no-issues rule gives `100%` directly).
...
- **Story-index source** — `story_points_total`: `45`, `story_plan_tasks`: `5`
  - counted from `harness-runs/story_plans/fix_observations_repo_relative_paths_story_plan.md` (the `N. [x] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike; all five are `[x]`).
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    - Task 1: `5` (layer `cli`)
    - Task 2: `10` (layer `plugin`)
    - Task 3: `15` (layer `plugin`)
    - Task 4: `10` (layer `plugin`)
    - Task 5: `5` (layer `general`)
    - **sum (`story_points_total`)**: `45`
  - `dispositioned_points`: `0` — no commit in `dev..HEAD` (`git log --format='%h %s%n%b' dev..HEAD`, 24 commits) carries the `record disposition of ` record, so no unit here was closed without a fix; no tokens matched.
...
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0`
  - glob `harness-runs/user_reviews/fix_observations_repo_relative_paths_review*.md` matched no files (pre-user-review write), so no enumeration arm was run.
  - **total**: `0` observations → `issue_cost_total` `0`
  - Each file's observation count, when files exist, is the count of whatever top-level enumeration that file uses, taken from the first of four arms to return non-zero and never summed across arms: (1) numbered `##`–`####` headings with required trailing punctuation, `grep -cE '^#{2,4} +[0-9]+[.):]'`; (2) top-level numbered/bulleted list items, `grep -cE '^[0-9]+\.|^[-*][[:space:]]'`; (3) an explicit per-observation marker the file itself uses, with the matching grep recorded beside the count; (4) `1`, reserved for a file with no enumeration of any kind.
