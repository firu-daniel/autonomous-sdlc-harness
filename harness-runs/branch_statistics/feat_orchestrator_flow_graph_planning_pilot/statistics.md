# Branch statistics: feat_orchestrator_flow_graph_planning_pilot

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `241` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
...
- `user_review_issues`: `0` — retained raw count of observations across all `feat_orchestrator_flow_graph_planning_pilot_review*.md` user-review files.
...
Computed: `round(clamp((241 - 0) / 241, 0, 1) * 100, 1)` = `100%`.
...
- **Story-index source** — `story_points_total`: `241`, `story_plan_tasks`: `17`
  - counted from `harness-runs/story_plans/feat_orchestrator_flow_graph_planning_pilot_story_plan.md` (the `N. [x] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike; all 17 are `[x]`).
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    ...
    - Task 16: `15`
    - Task 17: `10`
    - **sum (`story_points_total`)**: `241`
  - `dispositioned_points`: `0` — no commit in `dev..HEAD` (`git log --format='%h %s%n%b' dev..HEAD`) carries the `record disposition of ` record, so no unit was closed without a fix.
...
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0`
  - glob `harness-runs/user_reviews/feat_orchestrator_flow_graph_planning_pilot_review*.md` matched no files — this is the pre-user-review write, so there is nothing to count.
...
- status: `pre-user-review` — written right after the branch (code) review with no user review yet (`issue_cost_total` `0` → `100%`); `post-user-review` would mean it was re-written after user-review fixes, with the issues from the `feat_orchestrator_flow_graph_planning_pilot_review*.md` files counted and weighted.
