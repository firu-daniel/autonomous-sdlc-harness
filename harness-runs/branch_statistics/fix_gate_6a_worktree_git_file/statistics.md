# Branch statistics: fix_gate_6a_worktree_git_file

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `25` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
...
- `user_review_issues`: `0` — retained raw count of observations across all `fix_gate_6a_worktree_git_file_review*.md` user-review files.
...
Worked example: `issue_cost_total` is `0`, so the no-issues rule applies: `round(clamp((25 - 0) / 25, 0, 1) * 100, 1)` = `100%`.
...
- **Story-index source** — `story_points_total`: `25`, `story_plan_tasks`: `3`
  - counted from `harness-runs/story_plans/fix_gate_6a_worktree_git_file_story_plan.md` (the `N. [x] **Task K** — … _(layer: general)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike).
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    - Task 1: `10`
    - Task 2: `5`
    - Task 3: `10`
    - **sum (`story_points_total`)**: `25`
  - `dispositioned_points`: `0` — no commit in `main..HEAD` (`git log --format='%h %s%n%b' main..HEAD`) carries the `record disposition of ` record, so no task here was closed without a fix and no unpointed token was matched.
...
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0`
  - glob `harness-runs/user_reviews/fix_gate_6a_worktree_git_file_review*.md` matched no files (pre-user-review write), so there are no observations to count or weight.
...
- status: `pre-user-review` — this file was written right after the branch (code) review with no user review yet (`issue_cost_total` `0` → `100%`); it is re-written as `post-user-review` after user-review fixes, with the issues from the `fix_gate_6a_worktree_git_file_review*.md` files counted and weighted.
