# Branch statistics: chore_plugin_prefix_command_sweep

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `195` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
...
- `user_review_issues`: `0` — retained raw count of observations across all `chore_plugin_prefix_command_sweep_review*.md` user-review files.
...
Worked example: `round(clamp((195 - 0) / 195, 0, 1) * 100, 1)` = `100%` (no issues → `100%`).
...
- **Story-index source** — `story_points_total`: `195`, `story_plan_tasks`: `13`
  - counted from `harness-runs/story_plans/chore_plugin_prefix_command_sweep_story_plan.md` (the `N. [x] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike; all 13 are `[x]`).
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    ...
    - Task 12: `15`
    - Task 13: `20`
    - **sum (`story_points_total`)**: `195`
  - `dispositioned_points`: `0` — no commit in `dev..HEAD` (`git log --format='%h %s%n%b' dev..HEAD`, `dev` being `harness.config.json` → `defaultBranch`) carries the `record disposition of ` record, so no unit was closed without a fix; no tokens matched.
...
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0`
  - glob `harness-runs/user_reviews/chore_plugin_prefix_command_sweep_review*.md` matched no file (pre-user-review write; `_fix_plan` files excluded by pattern).
