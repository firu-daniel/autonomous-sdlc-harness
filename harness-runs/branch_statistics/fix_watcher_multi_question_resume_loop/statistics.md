# Branch statistics: fix_watcher_multi_question_resume_loop

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `185` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
- `issue_cost_total`: `0` — subtraction (sum of each user-review observation's severity weight, in story-point units).
- `story_plan_tasks`: `13` — retained raw count of `## Phase 2 Readiness — Ordered Fix List` task entries.
- `user_review_issues`: `0` — retained raw count of observations across all `fix_watcher_multi_question_resume_loop_review*.md` user-review files.
...
Worked example: `round(clamp((185 - 0) / 185, 0, 1) * 100, 1)` = `round(100.0, 1)` = `100%`.
...
- **Story-index source** — `story_points_total`: `185`, `story_plan_tasks`: `13`
  - counted from `harness-runs/story_plans/fix_watcher_multi_question_resume_loop_story_plan.md` (the `N. [x] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike; all 13 are `[x]`).
  - raw count: `awk '/## Phase 2 Readiness — Ordered Fix List/{f=1;next} /^## /{f=0} f' <story_index> | grep -cE '^[0-9]+\.\s*\[[ x]\]\s*\*\*Task'` → `13`.
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    ...
    - Task 12: `15`
    - Task 13: `5`
    - **sum (`story_points_total`)**: `185`
  - `dispositioned_points`: `0` — no commit in `dev..HEAD` (`git log --format='%h %s%n%b' dev..HEAD`, `defaultBranch` = `dev`) carries the `record disposition of ` record, so no task here was closed without a fix, and no unpointed token was matched.
