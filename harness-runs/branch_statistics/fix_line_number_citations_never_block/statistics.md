# Branch statistics: fix_line_number_citations_never_block
...
- `user_review_issues`: `0` — retained raw count of observations across all `fix_line_number_citations_never_block_review*.md` user-review files.
...
Worked example: `round(clamp((135 - 0) / 135, 0, 1) * 100, 1)` = `round(100.0, 1)` = `100%`.
...
- **Story-index source** — `story_points_total`: `135`, `story_plan_tasks`: `10`
  - counted from `harness-runs/story_plans/fix_line_number_citations_never_block_story_plan.md` (the `N. [x] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike; all ten are `[x]`).
...
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0` (per user-review file, then summed)
  - glob `harness-runs/user_reviews/fix_line_number_citations_never_block_review*.md` matched no files (the directory holds only `README.md`); this is the pre-user-review write, so no observations were counted or weighted.
...
- status: `pre-user-review` — written after the branch (code) review with no user review yet (`issue_cost_total` `0` → `100%`); `post-user-review` would mean it was re-written after user-review fixes, with the issues from the `fix_line_number_citations_never_block_review*.md` files counted and weighted.
