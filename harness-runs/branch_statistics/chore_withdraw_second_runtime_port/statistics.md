# Branch statistics: chore_withdraw_second_runtime_port

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `35` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
...
- `user_review_issues`: `0` — retained raw count of observations across all `chore_withdraw_second_runtime_port_review*.md` user-review files.
...
Worked example: `round(clamp((35 - 0) / 35, 0, 1) * 100, 1)` = `100%`.
...
- **Story-index source** — `story_points_total`: `35`, `story_plan_tasks`: `4`
  - counted from `harness-runs/story_plans/chore_withdraw_second_runtime_port_story_plan.md` (the `N. [ ] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike — all four are `[x]`).
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    - Task 1: `15`
    - Task 2: `10`
    - Task 3: `5`
    - Task 4: `5`
    - **sum (`story_points_total`)**: `35`
...
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0` (per user-review file, then summed)
  - glob `harness-runs/user_reviews/chore_withdraw_second_runtime_port_review*.md` matched no files (first write, pre-user-review); no enumeration arm was evaluated.
