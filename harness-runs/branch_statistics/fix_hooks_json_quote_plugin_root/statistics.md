# Branch statistics: fix_hooks_json_quote_plugin_root

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `20` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
- `issue_cost_total`: `0` — subtraction (sum of each user-review observation's severity weight, in story-point units).
- `story_plan_tasks`: `2` — retained raw count of `## Phase 2 Readiness — Ordered Fix List` task entries.
- `user_review_issues`: `0` — retained raw count of observations across all `fix_hooks_json_quote_plugin_root_review*.md` user-review files.
...
Worked example: `round(clamp((20 - 0) / 20, 0, 1) * 100, 1)` = `100%` (and the no-issues rule gives `100%` directly).
...
- **Story-index source** — `story_points_total`: `20`, `story_plan_tasks`: `2`
  - counted from `harness-runs/story_plans/fix_hooks_json_quote_plugin_root_story_plan.md` (the `N. [x] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike).
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    - Task 1 (layer `plugin`): `10`
    - Task 2 (layer `general`): `10`
    - **sum (`story_points_total`)**: `20`
  - `dispositioned_points`: `0` — no commit message in `main..HEAD` (`git log --format='%h %s%n%b' main..HEAD`) carries the `record disposition of ` record, so no unit was closed without a fix and no unpointed token was matched.
  - Back-compat fallback did not fire: both entries carry a `_(points: …)_` tag.
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0`
  - glob `harness-runs/user_reviews/fix_hooks_json_quote_plugin_root_review*.md` matched no file (first write, pre-user-review), so no per-file count, arm or grep applies.
