# Branch statistics: feat_harness_init_notification_topic_prompt

## Summary

- `success_rate`: **`100%`** — headline metric.
- `story_points_total`: `60` — denominator (sum of every task's `_(points: <N>)_` story-point estimate in the story index's `## Phase 2 Readiness — Ordered Fix List`).
...
- `user_review_issues`: `0` — retained raw count of observations across all `feat_harness_init_notification_topic_prompt_review*.md` user-review files.
...
Worked example: `round(clamp((60 - 0) / 60, 0, 1) * 100, 1)` = `100%`.
...
- **Story-index source** — `story_points_total`: `60`, `story_plan_tasks`: `4`
  - counted from `harness-runs/story_plans/feat_harness_init_notification_topic_prompt_story_plan.md` (the `N. [x] **Task K** — … _(layer: …)_ _(points: <N>)_` entries under its `## Phase 2 Readiness — Ordered Fix List`, counting `[ ]` and `[x]` alike; all four are `[x]`).
  - per-task points (`_(points: <N>)_` tag on each entry), summed:
    - Task 1: `20`
    - Task 2: `5`
    - Task 3: `20`
    - Task 4: `15`
    - **sum (`story_points_total`)**: `60`
  - `dispositioned_points`: `0` — no commit in `dev..HEAD` (`git log --format='%h %s%n%b' dev..HEAD`, 22 commits) carries the `record disposition of ` record, so no unit on this branch was closed without a fix.
...
- **User-review source** — `user_review_issues`: `0`, `issue_cost_total`: `0`
  - glob `harness-runs/user_reviews/feat_harness_init_notification_topic_prompt_review*.md` matched no files (pre-user-review write), so there are no per-file counts or costs.
