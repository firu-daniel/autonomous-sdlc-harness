...
**A drop becomes a pushed branch** with no queue server, webhook or scheduler in between. One file lands in `<state_dir>/autonomous_inbox/`, written by hand or by the conversational offer, and the next poll pass acts on it. With `execution.target` set to `github-actions`, the watcher dispatches that drop to a GitHub Actions job instead of launching it locally ([`docs/remote-execution.md`](docs/remote-execution.md)).
