...
| `transcripts/` | Arm A hand-run transcripts: one directory per corpus id, one file per variant and repetition (`<variant>-rep<N>.jsonl`), committed only after being read and redacted per the publication clearance, and scanned by gate 6e (`scripts/check-eval-artifacts.sh`). |
