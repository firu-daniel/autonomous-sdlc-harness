...
if [ -n "$port" ]; then
  case "$port" in
    *[!0-9]*)
      harness_fail "{{name}}: usage: start-dev-server.sh [port] (port must be numeric, got '${port}')"
      ;;
  esac
  shift
...
{{prestart}} || harness_fail "{{name}}: the pre-start step failed, so the server was not launched"
...
if ! kill -0 "$pid" 2>/dev/null; then
  harness_fail "{{name}}: the development server exited immediately — see $log_file"
fi
