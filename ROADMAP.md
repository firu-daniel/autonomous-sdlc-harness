...
| Cloud / CI execution | **The execution half has shipped:** with `execution.target` set to `github-actions` (opt-in; `local` stays the default), the local watcher dispatches each run to a GitHub Actions job on a hosted or self-hosted runner, and a run longer than one job runs as a chain of bounded jobs that each resume from the committed ledger. The docs-retrieval cache under `${XDG_CACHE_HOME:-$HOME/.cache}/autonomous-sdlc-harness/retrieval/` is restored per job, keyed on the CLI version, and seeded by a warm job on the default branch, since a cache saved on one feature branch is not readable from another ([`docs/remote-execution.md`](docs/remote-execution.md)). **Still open:** the trigger half — starting a run from issues, pull requests, comments, Jira and the like — which is the branch `feat_forge_run_triggers`, and draft-PR output, which belongs with that forge coupling; a remote run still ends at a pushed branch. Seam declared (`forge`): nothing reads it yet. | Open |
