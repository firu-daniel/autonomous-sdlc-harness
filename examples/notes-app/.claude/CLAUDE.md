...
notes-app is a browser notes list — add, edit, archive, pin, delete, and filter between active and archived — served as a static page and used by one person in one browser, with `localStorage` as the whole of its persistence. It has no runtime dependency and no UI framework: `package.json` declares TypeScript alone, `tsconfig.json` compiles `src/` to `dist/`, and `index.html` loads `dist/main.js` as a module. That shape binds before you touch a file. `src/main.ts` stays directly under `src/`, because `rootDir: "src"` is what emits it at the `dist/main.js` path `index.html` loads — moved into a layer directory it emits elsewhere, the page loads nothing, and the build still exits 0 (`src/main.ts`, file header). A static asset the page fetches stays at the repository root beside `index.html` and never under `src/`, because `tsc` compiles `.ts` and copies nothing else and the project has no bundler — an asset under `src/` never reaches `dist/` and 404s under `npm run dev` (`styles.css`, file header). And the suite runs against the **compiled** output rather than the sources: `pretest` builds first and each test file `await import`s from `dist/`, so a change to `src/` is invisible to a test run that did not rebuild (`package.json` (`scripts.pretest`), `test/notesService.test.mjs` (`await import('../dist/domain/notesService.js')`)).

_Merged by `/harness-analyze` in `existing` mode, from `package.json`, `tsconfig.json`, `index.html`, `styles.css`, `.gitignore` and the file headers under `src/`, with the compiled-suite claim re-read from `test/notesService.test.mjs` and `test/notesStore.test.mjs`._
...
|---|---|---|
| Data-layer module | `src/data/<camelCase>.ts` | `src/data/notesStore.ts` |
| Domain-layer module | `src/domain/<camelCase>.ts` | `src/domain/noteMapper.ts` |
| Presentation-layer module | `src/presentation/<camelCase>.ts` | `src/presentation/noteListView.ts` |
| Application entry point | `src/main.ts` — fixed name, directly under `src/`, never inside a layer directory | `src/main.ts` |
| Unit test | `test/<module under test>.test.mjs` — plain ESM, named for the module it exercises, matched by the `test` script's own glob | `test/notesService.test.mjs` |
| Test helper | `test/helpers/<camelCase>.mjs` | `test/helpers/storageDouble.mjs` |
| Dev tooling script | `tools/<kebab-case>.mjs` | `tools/dev-server.mjs` |
| Root static asset | `<name>.<ext>` at the repository root beside `index.html`, never under `src/` | `styles.css` |

_Merged by `/harness-analyze` in `existing` mode, from the tracked file names under `src/`, `test/` and `tools/` and at the repository root, from `package.json` (`scripts.test`) for the test glob, and from `styles.css` (file header) for where a fetched asset lives._
...
| The `data` layer — `src/data` | [`context/data-layer.md`](context/data-layer.md) |
| The `domain` layer — `src/domain` | [`context/domain.md`](context/domain.md) |
| The `presentation` layer — `src/presentation` | [`context/presentation.md`](context/presentation.md) |
| The `tests` layer — `test` | [`context/tests.md`](context/tests.md) |
