...
### Item 3a — walker state is machine-local, and a re-entry continues it

**Decision.** The walker's state lives at `<state_dir>/.flow_walker_state`, ignored beside `.dispatch_counter` (`cli/src/generators/repoRoot.ts` lists both; this checkout's `.gitignore` carries both). A planning (re-)entry issues `current` first and continues a pending walk (`plugin/instructions/task_plan_writing_instructions_core.md` → `## The walker — routing is its, judgement is yours` → **Continuing a saved walk.**). Only `start` overwrites the state and resets every counter to 0, and **proceed fresh**, **extend**, **review** and **skip** all issue it (`flow-walker.sh` → the header's `STATE` paragraph). The pilot reset the state on every re-entry; this reverses that.

**Reason.** The pilot's reset reproduced the pre-walker loop, which opened with *"`iteration = 0`. Loop:"* and held the counter only in context (`git show 1fd5ce9:plugin/instructions/task_plan_writing_instructions_core.md` → `## Loop`). With the step on disk, discarding it did harm. The autonomous fork's former case (b), *"Story index exists with a complete Phase 2 Readiness list"* (`git show 1fd5ce9:plugin/instructions/task_plan_writing_instructions_autonomous.md` → `## Override 2 — resumability`), skipped a draft no reviewer had passed, because a first draft already carries a complete readiness list. And `start` rewrote a finished UI-test plan with the `initial` prompt. So the counter now survives a pause, and the loop cap counts every revision of one walk, across every session it took.

**Which record wins.** The flow-progress ledger stays the only durable record, and it wins in both directions. A walk still in the task-plan loop is not used when `P1` is `[x]`. A walk past that loop is not used when `P1` is `[ ]`, unless its printed action carries `ledger: P1`; that flip is re-applied, idempotently through the commit wrapper's exit 3 (`task_plan_writing_instructions_autonomous.md` → `## Override 2 — resumability` → **Saved walk first.**, and `## Override 5 — pause/resume + flow-progress ledger (planning half)`). The fork checks a saved walk, never the walker: the walker reads no plan artifact, and reads nothing from the ledger but its run-mode block. A walk is unusable when the artifact its pending action works on is absent, which is what a draft deleted by hand leaves. A finished walk, `<escalate>` or `<terminal_handoff>`, is never acted on again; with `P3` `[ ]`, the ledger's own fallback reaches `--entry convergence`. A saved walk from another flow or branch is refused by `current` with exit 1.

**Evidence.** `cli/test/flow-walker-resume.test.mjs` pins continuation: cases `(1) a reviewer in flight is re-dispatched, and the counter and review series carry on`, `(2) a revision a FAIL queued is re-printed with its findings file`, `(3) a ledger line printed again flips nothing twice`, `(4) a UI reviewer in flight is re-dispatched, and the UI plan is not rewritten`, `(5) a parked <ask> resumes at the writer that parked, on its stored revision`, `(6) a finished walk is recognised as finished and replaced by the ledger fallback` and `(7) with no saved walk, a draft on disk is reviewed, never skipped or rewritten`. What `start` still resets is pinned by `cli/test/flow-walker-ui-and-reentry.test.mjs` → case `(6) re-entry mid-loop restarts the counter and continues the findings folder`: after `start` again, the heartbeat's counter is 0 and the architecture gate's `arg.iteration` is 1, because `review_0.md` is on disk.

**Cost.** A `P2` flip lost between the walker's print and the fork's flip, on a finished walk, is re-earned with one UI-review round rather than trusted from the saved walk. The semi-autonomous fork does not continue saved walks, because it keeps no ledger to check them against (`task_plan_writing_instructions_semi_autonomous.md` → its `<existing_artifact_decision>` row). The core's `## Setup` step 7 mapping of **extend** to `ui_writer`, added for the pilot's skeptic Finding 2 (`harness-runs/skeptic_reviews/feat_orchestrator_flow_graph_planning_pilot_skeptic_review/finding_2.md`), stays as the fallback, for a park at a loop's cap and for a lost state file.
...
## 3. Four findings established rather than assumed
...
- **The graph format and `scripts/check-flow-graph.sh` needed no change for the two new entries.** `business_parity_review` and `ui_review` are nodes `start` already reaches, so `reachable-from-start` and `reaches-terminal` hold, and no check reads `entries`. An entry that is not a node makes the walker fault at `start` with exit 2, which the core routes to `<escalate>`. `formatVersion` stays `1`, because the key set did not change; only the schema's `entries` description is restated, since a continued walk resumes without naming an entry.
