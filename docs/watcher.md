...
A run starts as a **file** and ends as a **notification**. There is no queue server, no webhook and no scheduler in between: a file appears in the inbox, and the next poll pass acts on it. Everything the watcher itself decides is in the eight steps below; everything a run then does — planning, implementing, reviewing, shipping — belongs to the engine command the drop is bound to, which the watcher launches and never inspects. That is the local path, and the default. With `execution.target` set to `github-actions`, the next poll pass still prepares the drop on this machine, then dispatches it to a GitHub Actions job instead of launching it here; the steps below mark where the two paths differ, and [`remote-execution.md`](remote-execution.md) is the design of record for everything after the dispatch.

1. **The drop.** One file is written into `<MAIN_REPO>/<state_dir>/autonomous_inbox/` — by `/autonomous-sdlc-harness:branch-prompt`, by `/autonomous-sdlc-harness:branch-user-review`, or by hand for a docs checklist, which no shipped command writes. `<MAIN_REPO>` is the first working copy `git worktree list` reports, and every central artifact lives under it — the inbox, the logs, the registry, the kill switch — whichever sibling working copy a run executes in.
...
3. **The guards, in one order.** The global kill switch `<state_dir>/AUTONOMOUS_STOP`, then the usage hold, then the per-repository concurrency cap (`MAX_PARALLEL_RUNS`), then a duplicate drop for a branch already running, then a `parked`, `paused` or `park_loop` record that already owns this branch's working copy, and **last**, only when `USAGE_LANE_LOCK_ENABLED=1`, the machine-level lane lock (§5) — taken immediately before the first step that creates anything, because taking it commits the whole machine to this repository. That knob is off by default, so as shipped the last guard passes without taking anything. The kill switch, the usage hold, the cap and the lane all **defer**: the file stays in the inbox, no record is written, and a later pass picks it up with nothing to re-drop by hand. The duplicate drop and the branch-owning record are **archived** instead, because waiting would not make either a different file — and the owning record is left byte-identical, so its own resume path still works. For a `park_loop` record the rejection names the `PARK_LOOP_CLEAR` clear action (§4) rather than an answer, because answering does not release it. A **remote** drop skips the usage hold, the cap and the lane, because the job gates itself — it runs the watcher's own usage gate on its own stream — and no local component gates a remote run; the kill switch still defers it, and a branch-owning record still rejects it. Before the duplicate test on a record carrying `execution: github-actions`, which has no local process to look at, the watcher refreshes that record's status with a best-effort `remote-run.sh sync`, so a still-`running` remote run makes the drop a duplicate.
...
5. **The artifact is placed, and for two of the three patterns committed — all three on a remote run.** A task prompt and a docs checklist are copied in and committed through `commit-on-branch.sh`, then pushed as a separate statement through `push-branch.sh`; a review file is placed and deliberately not committed, because the flow's own commits pick it up. Either commit path is non-blocking — a failed commit or push is one warning line in the run log and the run launches anyway. **A remote run commits all three**, and blocks on them. The job checks out `origin/<branch>` and sees only what was pushed, so a review left uncommitted would never reach it; the working copy is first fast-forwarded to `origin/<branch>`, because the job is where the branch advanced, and the review is committed as `chore: add user review for <branch>`. A failed commit, or a push that did not land, stops the dispatch with one `failed` notification. The inbox file is archived as `.processed/<timestamp>_<name>`.
6. **The engine is launched headless**, in a detached subshell whose working directory is the prepared copy: one `-p` session of the bound slash command, run with the generated permission profile through `--settings`, the model from `agentModel`, the effort level from `agentEffort` when that key is set, and `--output-format stream-json`. The raw events are teed to `<state_dir>/autonomous_logs/<branch>.stream.jsonl` — the copy the usage gate parses — and the same stream is formatted into the readable `<branch>.log` by `autonomous-format-stream.sh`. A **remote** drop is not launched: it is dispatched through `remote-run.sh dispatch`, which opens no live-log window and spawns nothing here, and its record carries `execution: github-actions` and no `pid`.
7. **The session's exit is classified** from the engine's own exit code plus the sentinels it left behind, into one of the five terminal states in the table below, recorded in `<state_dir>/autonomous_logs/registry.json`. A remote run's exit is classified in the job, by the same `classify_run_exit`, under `autonomous-watcher.sh job`; the local record learns the result only when `remote-run.sh sync` reads the job's state bundle.
8. **One notification per lifecycle event** — `launched`, `completed`, `parked`, `paused`, `failed`, `resumed`, `park_loop` — through `autonomous-notify.sh`, whose header is where that vocabulary is normative.
...
| `<branch>_review[_<n>].md` | `/branch-start-user-review-fix-autonomous` | the branch's existing copy when it is usable, else recreated for that branch | `<state_dir>/user_reviews/<the dropped filename>`, round suffix intact — **not committed** for a local run; committed as `chore: add user review for <branch>` and pushed for a remote one |
...
**The status vocabulary — one live state and five a session ends in.** The registry is a single object shaped `{"runs": {"<branch>": {…}}}` and this vocabulary is a contract — `/autonomous-sdlc-harness:branch-status`, `/autonomous-sdlc-harness:branch-answer`, `/autonomous-sdlc-harness:branch-pause`, `/autonomous-sdlc-harness:branch-resume` and `restart-watcher.sh` all read it, and `remote-run.sh` reads and writes it (`sync`, `stop`) — stated normatively in the watcher's own header. A record the watcher dispatched also carries the field `execution` with the value `github-actions`; every later pass reads that field, never the current `execution.target`, so a run keeps the execution it started with. `restart-watcher.sh` lists such records rather than refusing over them, since a restart cannot reach a job.

| `status` | Reached when | What ends it |
|---|---|---|
| `running` | the run was launched or resumed | one of the five below, or the reconcile pass when its process vanished |
...
| `parked` | the run wrote an unanswered clarification question and yielded | `/autonomous-sdlc-harness:branch-answer` writes the park's `answer_<n>.md`; the park-resume pass re-launches the same engine in the same working copy once every open question file is answered |
| `park_loop` | consecutive park-resumes made no progress (§4) | the operator's clear action (§4), after which the record is `parked` again |
| `paused` | the run honored a `PAUSE` request and yielded at a clean checkpoint (§4) | a `RESUME` sentinel — dropped by `/autonomous-sdlc-harness:branch-resume`, or by the usage gate for a run it paused itself |
...
|---|---|---|---|
| `lib/harness-run-lib.sh` | The one reader of `harness.config.json` for this family: anchors, state paths, the protected-branch answer, the machine-level lane primitives | sourced by every script row below; never executed, and it ships without an executable bit | no |
...
| `lib/flow-walker-gates.sh` | The walker's gate predicates | sourced by `flow-walker.sh`; never executed, and it ships without an executable bit | no |
| `flows/task_plan_writing.graph.json` | The planning loop's declared flow graph: its nodes, their outcomes, the `iteration` counter and its cap, the escalation payloads | read by `flow-walker.sh` from beside itself, by flow id; never executed | no |
| `flow-walker.sh` | Walks a declared flow graph and prints the planning loop's next action (`start`, `next`, `current`) | the orchestrating session of a planning flow | **yes** |
...
| `autonomous-watcher.sh` | The loop in §1; under `job`, exactly one run inside a remote job's own checkout | the service manager, or an operator for one `tick` / `status` / `usage`; the remote job for `job` | no — also **withheld** by the script-allowlist guard |
...
| `remote-run.sh` | Every call from this machine, or from a job, to GitHub for a harness run — locally `dispatch`, `pause`, `stop`, `sync`, `status` and `warm`, in the job `restore`, `save`, `continue` and `poll`, among others; it reads and writes the run registry (`sync`, `stop`). Its header states every verb and its exit map | the watcher, for a remote drop and each relay; the remote job; an operator by hand | no — also **withheld** by the script-allowlist guard |

**Why only the `yes` rows carry a profile entry, and why that is not the reachability answer.** The generated permission profile emits allow entries for exactly the rows marked yes, in the three literal forms a caller may spell them — that flag is declared once, in `cli/src/generators/outerLoopScripts.ts`, and the profile derives its entries from it, so a script cannot be allow-listed without a row saying it should be. The rest are run by the watcher process, the remote job, the agent runner or a person — the agent runner starts `docs-search-server.sh` from `.mcp.json` when `docs.retrieval` is on — except the walker's two: its gate library is sourced and its graph is read, both by `flow-walker.sh`, and neither is run. Four of the `no` rows (plus the deploy wrapper) are **withheld by basename from the script-allowlist guard's allow**. Being on that guard's deny list is not a refusal by it — it is the withdrawal of a permit: the guard grants nothing, emits nothing, and the invocation falls through to the permission profile, which carries no rule of any kind for these four and an `ask` for the deploy wrapper. So a dispatched agent that reaches for one gets a prompt it cannot answer, never a silent permit, and those five basenames stay unreachable **as commands**: no command string an unattended run composes starts a run about itself, bounces the daemon supervising it, reaches the sweep's `git branch -D`, or dispatches, pauses or cancels a remote run through `remote-run.sh`. **That is a bound on command strings, not on the run.** `scratch-run.sh` is agent-invocable and `exec`s an interpreter on a file the agent wrote a moment earlier, so what a deny-listed basename does is reachable from inside a probe — its own header says so under *WHAT THIS DOES NOT CONTAIN*, and `docs/outer-loop-verification.md` §3 records the reach beside the split it measures. The layer that holds for a subprocess is the `pre-push` git hook. **That guard auto-allows the `no` rows that are run and not withheld — `create-worktree.sh`, `setup-worktree.sh`, `autonomous-format-stream.sh`, `autonomous-notify.sh` and `docs-search-server.sh` — deliberately**, and `scratch-run.sh` beside them: its `yes` row carries no deny entry either, which is what reaches it on an adoption whose profile has never been regenerated. Those six are the left-allowed set `plugin/hooks/README.md` and the guard's own `DENY_SCRIPT_BASENAMES` comment both name. `flow-walker.sh` is reached by both mechanisms on the same ground as `scratch-run.sh` — a `yes` row with no deny entry — through that row rather than through that list. All of it is bounded by the construct scan exactly as the yes rows are: a `no` in this column withholds a profile entry, never reachability. `plugin/hooks/README.md`'s `## The deny list` is the long form, with the per-entry reasons; `docs/outer-loop-verification.md` §3 measures both halves of the split against the shipped guard.
...
**Parking is the same shape, on a different file.** A run that reaches a decision it must not take alone writes a `question_<n>.md` in its working copy's clarifications directory and ends its session; the watcher classifies that as `parked` from the question file alone, whatever the exit code — any numbered `question_<n>.md` still at the top level once the pairs a resume consumed are archived, answered or not, because an answered pair still there was written during the session and never read by it. A park is **one** question file holding every question of that park; a park written before that layout, as several files, follows the same rule. `/autonomous-sdlc-harness:branch-answer` lands the paired `answer_<n>.md`, and the park-resume pass re-launches the same engine once **every** open top-level question file has its answer, naming all of those answers in the resume prompt. It **leaves the pairs at the top level** where the re-entering run reads them while that session runs, and once it has exited archives **exactly that set** into `answered/`; a pair written during the resumed session stays at the top level and waits for the next resume. The protocol itself is the plugin's (`plugin/instructions/task_plan_writing_instructions_autonomous.md` → `## Clarification channel — file format (canonical, single source of truth)`). The watcher never writes an answer and never removes a question ahead of the run that has to consume it; a resume is also a launch as far as the kill switch, the cap and the lane are concerned, and a deferral there leaves every file exactly where the next pass expects it.

**A resume that makes no progress is bounded.** A session the park-resume pass launched that parks again is a **no-progress cycle** when either detector fires: it raised no question file above the highest index present at resume time, or it parked again less than `PARK_LOOP_WINDOW_SECS` (default `300`; `0` judges on the index alone) after the resume. Each cycle writes `resume made no progress (cycle <c>/<max>: <reason>)` to `watcher.log`. After `PARK_LOOP_MAX_CYCLES` (default `2`; `0` disables the guard) consecutive cycles the record becomes `park_loop` instead of `parked`, `watcher.log` carries `park loop — <c> consecutive resumes made no progress; not resuming it again`, one `park_loop` notification is sent, and no resume pass offers the run again. A progress cycle, a `completed` exit or a `failed` exit resets the count; a session the pause-resume pass launched is never counted. Answering a question does not release the hold. The clear action is to create the marker in the run's working copy, which `/autonomous-sdlc-harness:branch-status` prints with the real paths:

```
touch <worktree>/<state_dir>/clarifications/<branch>/PARK_LOOP_CLEAR
```

The next pass removes the marker, resets the count and sets the record back to `parked`, and the run resumes in that same pass when its park is answered.

The defaults are sized from this repository's own `watcher.log`, read on 2026-09-17: real park-resumed sessions ran 50m04s, 38m28s and 3h01m56s, and the spurious ones ran 41s and 39s, so a `300`-second window separates the two classes by a wide margin on both sides, and `2` cycles holds a loop after one repeat without holding a run whose single fast re-park was legitimate. The guard and the one-file-per-park layout exist because of the `feat_docs_catalog_retrieval` run that same day: it parked with three questions, was resumed once, parked again on `question_4.md`, and was then resumed twice more for answers it had already consumed — those two sessions are the 41s and 39s above. Branch `fix_watcher_multi_question_resume_loop` fixed it. This paragraph is the only place those timings are written.
...
Its policy knobs, by name: `USAGE_CHECK_ENABLED`, `USAGE_CHECK_INTERVAL_SECS`, `USAGE_PAUSE_TRIGGER`, `USAGE_WARNING_DEBOUNCE`, `USAGE_RESUME_MARGIN_SECS`, `USAGE_SEVEN_DAY_PAUSE_PCT`, `USAGE_LANE_STATE_ENABLED`, `USAGE_LANE_LOCK_ENABLED` — and, for the stall watchdog that shares the same hold marker, `STALL_CHECK_ENABLED`, `STALL_WARN_SECS`, `STALL_KILL_SECS`, `STALL_MAX_RESTARTS`, `STALL_BUSY_CPU_PCT` — and, for the park-loop guard above, `PARK_LOOP_MAX_CYCLES`, `PARK_LOOP_WINDOW_SECS`. **Their defaults are the watcher's**, stated in its header and printed live by `autonomous-watcher.sh status`; that command's output is the answer to what a given machine is actually running with.
...
**A remote run gets these mechanisms in the job, not from this daemon.** The usage gate, the stall watchdog and the park-loop guard run **inside the job**, as the watcher's own code under `autonomous-watcher.sh job`, on the job's own stream and knobs mapped from repository variables. The local passes skip every record carrying `execution: github-actions`: the vanished-process reconcile, the cap count, the stall watchdog and both halves of the usage gate. A remote run's pause and resume reach it only as **relayed dispatches**, each a user's action written into the mirror working copy: a `PAUSE` on a `running` record becomes `remote-run.sh pause`, a `RESUME` becomes `remote-run.sh dispatch --resume pause`, a fully answered park becomes `remote-run.sh dispatch --resume answer`, and a `PARK_LOOP_CLEAR` rides along as `--park-loop-clear`. The kill switch defers the two resumes; the pause relay runs ahead of it, the one action taken under the brake, because it starts nothing. The machine lane stays local: job mode turns both halves off. What the job does after its own gate pauses a run is [`remote-execution.md`](remote-execution.md) → `### Resuming without the local watcher`.
...
**2. Configure notifications, or accept getting none.** Delivery is **opt-in and defaults to nothing pushed**: the shipped example (`cli/templates/claude/push-notify.env.example`) ships both keys empty, and with neither filled the notifier degrades to a desktop banner where one is available and still exits 0. Fill them in one of two places, in this order — the machine-local `${XDG_CONFIG_HOME:-$HOME/.config}/autonomous-sdlc-harness/push.env` first, the repository's configured `pushEnvPath` second; the first that exists wins and the other is not read. Machine-local comes first on purpose: a push token belongs to a person and a machine rather than to a checkout, and from there it cannot be committed by accident. **Filling that machine-local file is no longer only a hand step: `init` writes it for you from either an ntfy topic name or the full URL of any endpoint that accepts a POST**, and on a terminal `init` asks (`docs/cli.md` §2):

```bash
npx autonomous-sdlc-harness init --notifications --push-url <url-or-ntfy-topic>
```

Given the opt-in and no endpoint it writes nothing at all and prints the setup instead — an empty machine-local file would win this very precedence over a `pushEnvPath` that already has values. Writing either file by hand stays exactly as supported as it was, and by hand the file takes a full URL: an ntfy topic is written `https://ntfy.sh/<your-topic>`, and a URL containing shell characters such as `&` is single-quoted, because the file is sourced. The notifier itself is unchanged by the topic route, because what it reads is always a URL. The keys, what each delivery arm does with them and how a value already in the environment interacts with the file are `autonomous-notify.sh`'s header and that example file; nothing here restates them. Desktop banners are macOS-only and degrade to nothing on a Linux host, so on a systemd machine the push arms are the notification.
...
**Remote execution adds steps only the adopter can take, on GitHub** — committing the two workflows to the default branch, setting a credential secret, choosing a runner — and [`remote-execution.md`](remote-execution.md) → `## 7. Turning it on` lists them. `doctor`'s `remote-execution` check reports whether the ones visible from the checkout are owed, and `doctor --check-github` asks GitHub about the secrets and variables (`docs/cli.md` §7). The daemon from step 1 is still needed: it is what dispatches a remote run's start and relays each answer, resume and pause, but nothing local has to be running while the job executes.
