...
7. **The session's exit is classified** from the engine's own exit code plus the sentinels it left behind, into one of the five terminal states in the table below, recorded in `<state_dir>/autonomous_logs/registry.json`.
