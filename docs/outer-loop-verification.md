...
**A real GitHub Actions run.** Every remote-execution case — `remote-run.sh`'s verbs, job mode, the local watcher's dispatch and relays, and `doctor`'s `remote-github` check — is driven against a `gh` stub named by `HARNESS_GH_CLI`, and job mode against the agent stub as well (`cli/test/remote-run.test.mjs`, `watcher-remote-job.test.mjs`, `watcher-remote-dispatch.test.mjs`, `remote-names.test.mjs`, `doctor.test.mjs`). So the **commands the harness issues and how it reads their answers are exercised; what GitHub does with them is not**, and nothing here shows any of five behaviours `docs/remote-execution.md` → `## 6. What is not verified here` rests on. GitHub accepting a dispatch: a workflow it rejects — the harness step's expression-valued `timeout-minutes` is the likeliest cause — passes every case and starts no run. A `GITHUB_TOKEN` dispatch starting a new run: if it does not, a long run stops at its first self-pause instead of chaining. The poller's enable and disable under that token: if they fail, a usage-paused run waits for `/autonomous-sdlc-harness:branch-resume` rather than resuming. A job's time limits: if the 6-hour and 5-day figures are wrong, the time budget is mis-sized and a job is killed mid-session or self-pauses early. A skipped job's cost: if a skipped job bills, each `harness pause` and `harness stop` marker costs one minute on a private repository. Gate 12 in `docs/development.md` → `## 5. Verifying a change` is the hand-run that records each against a real repository.
