...
- **The published package declares no runtime dependency.** `cli/package.json` carries `devDependencies` and no `dependencies` key, and every `import` in `cli/src` resolves to a `node:` builtin or to a relative sibling. Adding the first runtime dependency is a decision raised as a `stale-rule` entry for a supervised `/harness-analyze` re-run or a hand edit, never a change an implementing run makes: a hand-rolled flag parser and a hand-written configuration check exist because of it (`cli/src/cli.ts` → `GLOBAL_OPTIONS`; `cli/src/config/check.ts` → *"This is not a schema validator, and it must never grow into one"*).
