...
_Written by `/autonomous-sdlc-harness:harness-analyze` in `existing` mode, from `README.md`, `package.json`, `cli/package.json`, `cli/src/cli.ts`, `plugin/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `examples/notes-app/README.md` and the repository layout; the audience and self-adoption blocks reduced to pointers by the corpus pass._
...
_Written by `/autonomous-sdlc-harness:harness-analyze` in `existing` mode, from the real file names under `cli/src`, `cli/test`, `cli/templates`, `plugin/agents`, `plugin/commands`, `plugin/instructions`, `plugin/hooks`, `plugin/samples`, `docs`, `schemas` and the repository root._
